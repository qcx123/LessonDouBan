...
- (void)openDB{
    ...
    if (result == SQLITE_OK) {
        NSLog(@"打开数据库成功");
        NSString *createSql = @"CREATE TABLE IF NOT EXISTS ActivityList (userName TEXT,ID TEXT , title TEXT, imageUrl TEXT, data BLOB);CREATE TABLE MovieList (userName TEXT,ID INTEGER AUTOINCREMENT, title TEXT, imageUrl TEXT, data BLOB)";
        NSString *createMovieSql = @"CREATE TABLE IF NOT EXISTS MovieList (userName TEXT,ID TEXT, title TEXT, imageUrl TEXT, data BLOB)";
        sqlite3_exec(db, [createSql UTF8String], NULL, NULL, NULL);
        sqlite3_exec(db, [createMovieSql UTF8String], NULL, NULL, NULL);
    }
...
- (MovieDetialModel *)selectMovieWithId:(NSString *)ID{
    ...
    if (result == SQLITE_OK) {
        ...
        if (sqlite3_step(stmt) == SQLITE_OK) {
            ...
            model = [[FileDataHandle shareInstance] unarchiverObject:data forKey:archiverKey];
        }
    }
    sqlite3_finalize(stmt);
    return model;
...
- (NSMutableArray *)selectAllMovie{
    ...
    if (result == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            MovieDetialModel *model = [[FileDataHandle shareInstance] unarchiverObject:data forKey:archiverKey];
